...
function SizePill({ label }: { label: string }) {
  return (
    <span className="inline-flex items-center gap-1 rounded-full bg-gray-100 text-gray-500 px-2 py-0.5 text-[11px] font-medium tracking-wide">
      ...
      <div className="relative w-full rounded-xl overflow-hidden bg-gray-50 ring-1 ring-gray-200/70" style={{ aspectRatio: String(ratio) }}>
        {currentUrl ? (
          <>
            ...
            <span className="absolute bottom-2 right-2 rounded-md bg-black/45 px-1.5 py-0.5 text-[10px] font-medium text-white/90 backdrop-blur-sm">建議 {sizeLabel}</span>
          </>
        ) : (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-1.5 text-gray-400">
            ...
            <span className="text-xs font-medium text-gray-500">建議 {sizeLabel}</span>
          </div>
        )}
      </div>
